...
> - [ ] for each OpenAPI/JSON Schema validation keyword, show exactly how that would be expressed in `ytt` validation.
...
- [maximum](https://datatracker.ietf.org/doc/html/draft-wright-json-schema-validation-00#section-5.2)
- exclusiveMaximum
- minimum
- exclusiveMinimum
- maxLength
...
> **TODO:**
- [ ] consider how to allow for or translate `ytt` validations to CEL rules.
...
Defines what a valid value is for the value of the annotated node.
...
- `when_null_skip=` (`bool`) — a special-case form of `when=` that checks if the value of the annotated node is `null`.
...
_Example 2: Custom violation message_
...
_Example 3: Custom predicate-based rule_
...
Some preliminary work has been done to augment the Overlay module to better support editing of annotations.  (see [Augmenting the Overlay module to edit @assert/validate annotations](#augmenting-the-overlay-module-to-edit-schemavalidate-annotations) for details).
...
- `validation_func` (`function`) — zero-to-many "[validation functions](#validation-functions)"
    ...
    - validator: `func () : {"description": string, "type": string}`
        - a "predicate" or "assertion" that when invoked with no arguments returns a [specification of the validator](#validation-function-specification).
...
When exploring how to implement [@assert/validate-string-defaults](#schemavalidate-string-defaults), there are challenges.
